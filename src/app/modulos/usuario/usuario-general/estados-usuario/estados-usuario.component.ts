...
@Component({
  ...
  templateUrl: './estados-usuario.component.html',
  standalone: true,
  imports: [
    ButtonModule,
    CheckboxModule,
  ...
  styleUrls: ['./estados-usuario.component.scss']
})
export class EstadosUsuarioComponent {
  ...
  protected actionType: 'desbloquear' | 'bloquear'; // 'desbloquear' or 'bloquear'
  ...
  ngOnInit(): void {
    this.actionType = this.config.data.actionType; // 'desbloquear' or 'bloquear'
    this.isDesbloqueado = this.actionType == 'desbloquear';

    this.userRow = { ...this.config.data };
    this.userDetail = Object.create(null);
    this.usuarioSesion = this.userService.getUserInfo();
    this.initializeForm();
    this.getUsuarioDetalle(this.userRow.idVUsuario);
  }
