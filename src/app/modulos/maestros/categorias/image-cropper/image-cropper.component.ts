...
import {
  OutputFormat,
  OutputType,
} from './interfaces/cropper-options.interface';
...
export class MyImageCropperComponent implements OnChanges, OnInit {
  ...
  @Input() resetCropOnAspectRatioChange =
  ...
  constructor(
    ...
    @Optional()
    @Inject(HAMMER_LOADER)
    private readonly hammerLoader: HammerLoader | null
  ) {
    this.reset();
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    this.handleLoadedImageTransform(changes);
    this.handleCropperChanges(changes);
    this.handleTransformChanges(changes);
    this.handleHiddenChanges(changes);
  }

  private handleLoadedImageTransform(changes: SimpleChanges) {
    if (
  ...
  private handleCropperChanges(changes: SimpleChanges) {
    if (
      changes['cropper'] ||
      ...
      if (this.shouldResetCropperPosition(changes)) {
  ...
  private handleTransformChanges(changes: SimpleChanges) {
  ...
  private handleHiddenChanges(changes: SimpleChanges) {
  ...
  private shouldResetCropperPosition(changes: SimpleChanges): boolean {
    return (
      this.maintainAspectRatio &&
      (this.resetCropOnAspectRatioChange || !this.aspectRatioIsCorrect()) &&
      (Boolean(changes['maintainAspectRatio']) || Boolean(changes['aspectRatio']))
    );
  }
  ...
  private setCssTransform() {
    ...
    this.safeTransformStyle = this.sanitizer.bypassSecurityTrustStyle(
      `translate(${this.transform.translateH || 0}${translateUnit}, ${this.transform.translateV || 0
      }${translateUnit})` +
      ' scaleX(' +
  ...
  private setLoadedImage(loadedImage: LoadedImage): void {
    ...
    this.safeImgDataUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
      loadedImage.transformed.objectUrl
    );
    this.cd.markForCheck();
  }
  ...
  private async activatePinchGesture() {
    ...
    const Hammer =
      (window as unknown as Window & { Hammer?: HammerStatic })?.['Hammer'] ||
      null;
  ...
  private resizeCropperPosition(oldMaxSize: Dimensions): void {
    if (
      ...
      this.cropper.x1 =
        (this.cropper.x1 * this.maxSize.width) / oldMaxSize.width;
      this.cropper.x2 =
        (this.cropper.x2 * this.maxSize.width) / oldMaxSize.width;
      this.cropper.y1 =
        (this.cropper.y1 * this.maxSize.height) / oldMaxSize.height;
      this.cropper.y2 =
        (this.cropper.y2 * this.maxSize.height) / oldMaxSize.height;
    }
  }
  ...
  crop(
    output: OutputType = this.settings.output
  ): Promise<ImageCroppedEvent> | ImageCroppedEvent | null {
  ...
  private cropToBlob(): Promise<ImageCroppedEvent> | null {
    const result = this.cropService.crop(
      this.loadedImage,
      this.cropper,
      this.settings,
      'blob',
