...
export class CropperPositionService {
  resetCropperPosition(
    ...
    if (settings.cropperStaticHeight && settings.cropperStaticWidth) {
      ...
      cropperPosition.x2 =
        maxSize.width > settings.cropperStaticWidth
          ? settings.cropperStaticWidth
          : maxSize.width;
      cropperPosition.y1 = 0;
      cropperPosition.y2 =
        maxSize.height > settings.cropperStaticHeight
      ...
      if (!settings.maintainAspectRatio) {
        ...
        cropperPosition.y1 =
          (maxSize.height - cropperHeightWithAspectRatio) / 2;
  ...
  checkAspectRatio(
    ...
    switch (position) {
      case 'top':
      case 'bottom':
        this.ajustarTopBottom(position, cropperPosition, maxSize, settings);
        break;
      case 'topleft':
      case 'topright':
        this.ajustarTopRightLeft(position, cropperPosition, maxSize, settings);
        break;
      case 'right':
      case 'bottomright':
      case 'left':
      case 'bottomleft':
        this.ajustarLeftRightBottom(cropperPosition, maxSize, settings);
        break;
      case 'center':
        this.ajustarCenter(cropperPosition, maxSize, settings);
        break;
    }
  }

  private ajustarTopBottom(
    position: string,
    cropperPosition: CropperPosition,
    maxSize: Dimensions,
    settings: CropperSettings
    ...
    const overflowX = Math.max(cropperPosition.x2 - maxSize.width, 0);
    const overflowY = Math.max(position === 'top' ? 0 - cropperPosition.y1 : cropperPosition.y2 - maxSize.height, 0);
    this.ajustarOverflow(cropperPosition, overflowX, overflowY, settings);
  }

  private ajustarTopRightLeft(
    position: string,
    cropperPosition: CropperPosition,
    maxSize: Dimensions,
    settings: CropperSettings
    ...
    const overflowX = Math.max(position === 'topright' ? cropperPosition.x2 - maxSize.width : 0 - cropperPosition.x1, 0);
    const overflowY = Math.max(0 - cropperPosition.y1, 0);
    this.ajustarOverflow(cropperPosition, overflowX, overflowY, settings);
  }

  private ajustarLeftRightBottom(
    cropperPosition: CropperPosition,
    maxSize: Dimensions,
    settings: CropperSettings
  ): void {
    ...
    const overflowX = Math.max(cropperPosition.x2 - maxSize.width, 0);
    const overflowY = Math.max(cropperPosition.y2 - maxSize.height, 0);
    this.ajustarOverflow(cropperPosition, overflowX, overflowY, settings);
  }

  private ajustarCenter(
    cropperPosition: CropperPosition,
    maxSize: Dimensions,
    settings: CropperSettings
  ): void {
