...
import { IDatosLogo, IAgregarLogoCategoriaRequest, IEliminarLogoCategoriaRequest } from "@interfaces/categorias/categorias";
...
export class ModalLogoComponent {
  formGroup: FormGroup;
  options = [
  ...
  setData() {
    if (this.config) {
      this.nombre = this.config.data.dataEnvio.nombre;
      this.identificador = this.config.data.dataEnvio.idCategoria;

      this.logoPrevio.base64 = this.config.data.dataEnvio.logo;
      this.logoPrevio.extension = this.config.data.dataEnvio.extension;

      if (!this.logoPrevio.base64)
        this.optionSelectedField.disable();

      this.infoUsuario = this.config.data.infoUsuario;
    }
  }
  ...
  mostrarAlertConfirmacion(action: 'agregar' | 'eliminar') {
    ...
    if (action === 'agregar') {
      ...
      description = `A continuación, se procederá a registrar el nuevo logo de la categoría <strong>${this.nombre}</strong>. ¿Está seguro de realizar esta acción?`;
    } else if (action === 'eliminar') {
      title = 'Eliminar Logo';
      description = `A continuación, se procederá a eliminar el logo de la categoría <strong>${this.nombre}</strong>. ¿Está seguro de realizar esta acción?`;
    }
  ...
  onAgregarLogo() {
    const datosSolicitud: IAgregarLogoCategoriaRequest = {
      idCategoria: this.identificador,
      nombreLogo: this.datosLogoSeleccionado.nombre,
      tamanoLogo: this.datosLogoSeleccionado.tamano,
      extensionLogo: this.datosLogoSeleccionado.extension === 'jpg' ? 'png' : this.datosLogoSeleccionado.extension,
      imagenLogo: this.datosLogoSeleccionado.base64,
      codigoUserName: this.infoUsuario.usuario,
    };

    this.categoriaService.agregarLogoCategoria(datosSolicitud).subscribe({
      next: () => {
        setTimeout(() => {
          this.modalNotificationService.dialogSuccess(
            'Logo registrado',
            `El registro de los datos del nuevo logo de la categoría <strong> ${this.nombre} </strong> se realizó de forma exitosa.`
          );

          this.categoriaService.callRefreshTable();
          this.closeModal();
        }, 300);
      },
      error: (err: string) => {
  ...
  onEliminarLogo() {
    const request: IEliminarLogoCategoriaRequest = {
      idCategoria: this.identificador,
      codigoUserName: this.infoUsuario.usuario,
    };
    this.categoriaService.eliminarLogoCategoria(request).subscribe({
      next: (res: any) => {
        setTimeout(() => {
          this.modalNotificationService.dialogSuccess(
            'Logo eliminado',
            `La eliminación del logo de la categoria <strong>" ${this.nombre}"</strong> se realizó de forma exitosa.`
          );
  ...
  fileChangeEvent(event: any) {
    ...
    if (file) {
      ...
      if (!validFormats.includes(extension)) {
        this.errorMessage =
          'Formato no permitido. Solo se aceptan archivos JPG, JPEG y PNG.';
        return;
      }
      ...
      this.datosLogoSeleccionado.nombre = file.name.substring(
        0,
